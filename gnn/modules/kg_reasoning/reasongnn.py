...
class ReasonGNNLayer(BaseGNNLayer):
    ...
    def forward(self, current_dist, relational_ins, step=0, return_score=False):
        ...
        for j in range(relational_ins.size(1)):
            ...
            neighbor_rep, forward_paths = self.reason_layer(current_dist, relational_ins[:,j,:], rel_linear, pos_emb, return_path=True)
            neighbor_reps.append(neighbor_rep)
            path_info[f'forward_{j}'] = forward_paths

            neighbor_rep, backward_paths = self.reason_layer_inv(current_dist, relational_ins[:,j,:], rel_linear, pos_emb_inv, return_path=True)
            neighbor_reps.append(neighbor_rep)
            path_info[f'backward_{j}'] = backward_paths 
