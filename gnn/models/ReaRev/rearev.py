...
ReaRev模型结构
继承BaseModel, 主要功能有
1. 问题编码, 即将自然语言query转化为向量表示
2. kg推理, 通过GNN对entity和relation推理, 找到与问题有关的entity
3. 根据推理结果生成答案分布并计算loss进行训练
"""
...
class ReaRev(BaseModel):
    def __init__(self, args, num_entity, num_relation, num_word):
        ...
        self.loss_type =  args['loss_type'] # loss类型
        self.num_iter = args['num_iter']
        self.num_ins = args['num_ins'] # 指令生成的次数
        self.num_gnn = args['num_gnn'] # GNN的层数
        self.alg = args['alg'] # 推理算法，如BFS
        assert self.alg == 'bfs'
        self.lm = args['lm'] # LSTM或BERT
        
        self.private_module_def(args, num_entity, num_relation)

        self.to(self.device)
        self.lin = nn.Linear(3*self.entity_dim, self.entity_dim) # 线性层，用于调整嵌入维度
    ...
    def forward(self, batch, training=False): # 模型的核心逻辑
        """
        ...
        self.init_reason(curr_dist=current_dist, local_entity=local_entity,
                         kb_adj_mat=kb_adj_mat, q_input=q_input, query_entities=query_entities) # 初始化推理过程
        self.instruction.init_reason(q_input)
        for i in range(self.num_ins):
            # gnn/modules/question_encoding/base_encoder.py, 通过语言模型生成初始instruction
